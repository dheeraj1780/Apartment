...
class Update_profile:
    def __init__(self,root,name,username):
        ...
        name_txt = Label(f2,text="Name :", font=("Roboto", 16, "bold"),fg="#FFD700",bg="#222222")
        name_txt.place(relx=0.4,rely=0.05)
        ...
        #getting the details of the resident with username
        print(f"SELECT Password,Phone,BlockNo, HouseNo, Join_Date FROM resident WHERE Username = '{self.username}'")
        self.cursor.execute("SELECT Password, Phone, BlockNo, HouseNo, Join_Date FROM resident WHERE Username = '"+self.username+"'")
        ...
        self.house_label = Label(f2,text=self.details[0][3],font=("Verdana",12 ,"bold"),fg="#FFD700",bg="#111111")
        self.house_label.place(relx=0.6,rely=0.15,height=30,width=350)

        self.block_label = Label(f2,text=self.details[0][2],font=("Verdana",12 ,"bold"),fg="#FFD700",bg="#111111")
        ...
        self.password_entry.insert(0, self.details[0][0])
        ...
        self.phone_entry.insert(0, self.details[0][1])
        ...
        self.date_entry.insert(0, self.details[0][4])
    ...
    def update_records(self):
        ...
        houseno = self.details[0][3]
        blockno = self.details[0][2]
        date = self.date_entry.get()
        if (name=="" or username =="" or password=="" or phone=="0" or date==""):
            tmsg.showerror("EMPTY ENTRY","Please fill the entries")
        else:
            self.cursor.execute(f"UPDATE resident SET Name = %s ,Username = %s ,Password = %s ,Phone = %s ,BlockNo = %s ,HouseNo = %s ,Join_Date = %s WHERE Username='{self.username}'",(name,username,password,phone,blockno,houseno,date))
            self.conn.commit()
            release_connection(self.conn)
            self.root.destroy()
            from Controller import present_Userpage_gui_frame
            present_Userpage_gui_frame(name,self.username)
